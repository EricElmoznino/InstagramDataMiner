...
class HashTagSearch(metaclass=ABCMeta):
    ...
    def extract_recent_tag(self):
        ...
        for potential_id in potential_query_ids:
            url = 'https://www.instagram.com/graphql/query/?query_id=%s&variables={"tag_name":"%s","first":12,"after":"%s"}' % \
    ...
    def get_query_id(self, doc):
        ...
        for script in doc.find_all("script"):
            if script.has_attr("src"):
                ...
                for query_id in re.findall('(?<=queryId:\")[0-9]{17}', text):
...
class HashTagDataMiner(HashTagSearch):
    def __init__(self, hashtag):
    ...
    def save_results(self, instagram_results):
        ...
        for post in instagram_results:
            ...
            if post.display_src is None: continue

            image = requests.get(post.display_src).content
            image = io.BytesIO(image)
            image = Image.open(image)

            pyplot.imshow(image)
            keep = pyplot.waitforbuttonpress()
            pyplot.draw()
            if keep:
                image.save(os.path.join('data', self.hashtag,
                                        ''.join(random.choice(string.digits) for _ in range(15)) + '.jpg'))
            pyplot.close()


if __name__ == '__main__':
    log.basicConfig(level=log.INFO)
    HashTagDataMiner('nomakeup').extract_recent_tag()
